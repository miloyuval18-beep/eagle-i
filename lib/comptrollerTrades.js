...
const ALL_NAICS = Object.values(TRADES).map(t => t.naics);
...
async function fetchTrades({ log = () => {} } = {}) {
  ...
  const countyList = HOUSTON_METRO_COUNTY_CODES.map(c => `'${c}'`).join(',');
  ...
  for (const raw of all) {
    ...
    const key = `${row.taxpayerNumber}|${row.naicsCode}`;
  ...
  for (let i = 0; i < rows.length; i += CHUNK) {
    ...
    const placeholders = chunk.map((r, j) => {
      ...
      values.push(
        ...
        r.outletCity, r.outletZip, r.outletCountyCode, r.permitIssueDate, r.firstSalesDate
      );
    ...
    await query(
      `INSERT INTO comptroller_trades
         (taxpayer_number, naics_code, taxpayer_name, outlet_name, outlet_address, outlet_city,
          outlet_zip, outlet_county_code, permit_issue_date, first_sales_date)
       VALUES ${placeholders.join(',')}
       ON CONFLICT (taxpayer_number, naics_code) DO UPDATE SET

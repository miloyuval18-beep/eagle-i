...
async function sendEmail({ to, subject, html, text, replyTo, fromName, fromAddress, bcc, headers }) {
  ...
  const defaultName = defaultFrom.replace(/<[^<>]*>\s*$/, '').trim().replace(/^"(.*)"$/, '$1') || null;
  ...
  // or break the header. Falls back to the default display name (not just
  // dropping it) so a caller that passes fromAddress without fromName still
  // gets a real name attached — previously this silently discarded
  // fromAddress entirely and fell back to the shared default sender.
  const displayName = fromName || defaultName;
  const quotedName = displayName ? `"${displayName.replace(/[\\"]/g, '\\$&')}"` : null;
  const from = quotedName ? `${quotedName} <${senderAddress}>` : senderAddress;
  ...
  // Lets a tenant with a verified sending domain see a real copy of their
  // own outreach land in an inbox they actually check, instead of only
  // being visible in Eagle I's dashboard or Resend's own logs.

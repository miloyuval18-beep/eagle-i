...
async function getSenderContext(tenantId) {
  ...
  const profileRes = await query('SELECT founder_name, phone, email, address, site, linkedin_url FROM business_profile WHERE tenant_id = $1', [tenantId]);
  return { companyName: tenantRes.rows[0].company_name, profile: profileRes.rows[0] || {}, fromAddress: await require('./sendingDomain').getVerifiedFromAddress(tenantId) };
}
...
// A subject is one line: control characters and line breaks would let text
// smuggle in extra headers, so they are removed.
function cleanSubject(subject) {
  return String(subject == null ? '' : subject).replace(/[\u0000-\u001f\u007f]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 150);
}
...
async function screenRecipients(tenantId, recipients) {
  ...
  for (const r of recipients || []) {
    ...
    seen.add(email);
    candidates.push({ email, name, source: r.source || null, sourceId: r.sourceId ? Number(r.sourceId) : null });
  }
  ...
  // Businesses the user has flagged "already my vendor" are not cold-emailed.
  const mine = new Set();
  const tracked = candidates.filter(c => c.source && c.sourceId);
  if (tracked.length) {
    const rel = await query(
      'SELECT source, source_id FROM vendor_relationships WHERE tenant_id = $1 AND is_my_vendor AND source = ANY($2) AND source_id = ANY($3)',
      [tenantId, [...new Set(tracked.map(c => c.source))], [...new Set(tracked.map(c => c.sourceId))]]);
    for (const x of rel.rows) mine.add(`${x.source}:${Number(x.source_id)}`);
  }
  const stillOk = [];
  for (const c of candidates) {
    ...
    else if (c.source && c.sourceId && mine.has(`${c.source}:${c.sourceId}`)) skipped.push({ ...c, reason: 'already_my_vendor' });
    else stillOk.push(c);
  }
...
// `meta` records what the email was aimed at, for results reporting and the
// relationship tracker: { kind, variant, testId, categoryKey, city, source, sourceId }.
async function sendOutreach({ tenantId, ctx, baseUrl, toEmail, vendorName, message, subject, meta = {} }) {
  ...
  const finalSubject = cleanSubject(subject) || `Quick note from ${ctx.companyName}`;
  let sent, sendError;
  try {
    sent = await sendEmail({
      to: email, subject: finalSubject, html, text, replyTo, fromName: ctx.companyName, fromAddress: ctx.fromAddress,
      // BCC the tenant's own verified sending address (when they have one) so
      // a real copy lands in an inbox they actually check, not just the
      // dashboard. No verified domain means no real inbox to BCC into.
      bcc: ctx.fromAddress || undefined,
  ...
  await query(
    `INSERT INTO vendor_outreach (id, tenant_id, vendor_name, to_email, message, status, error, resend_email_id,
                                  subject, kind, variant, test_id, category_key, city, source, source_id)
    ...
    [outreachId, tenantId, vendorName || email, email, message,
     ...
     finalSubject, meta.kind || 'initial', meta.variant || null, meta.testId || null, meta.categoryKey || null,
     meta.city || null, meta.source || null, meta.sourceId || null]
  );
  if (sendError) return { ok: false, reason: 'send_failed', error: sendError.message, outreachId };
  if ((meta.kind || 'initial') === 'initial' && meta.source && meta.sourceId) {
    // Tracking must never turn a delivered email into an error.
    await require('./relationships').noteOutreach({ tenantId, source: meta.source, sourceId: meta.sourceId, vendorName: vendorName || email, email })
      .catch(err => console.error('[relationships] noteOutreach failed:', err.message));
  }
...
module.exports = {
  ...
  cleanSubject, getSenderContext, composeEmail, screenRecipients, sentInLast24h, sendOutreach
};

...
async function generateJSON(prompt, maxTokens = 1500) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    // Thinking explicitly disabled: these calls only want a structured JSON
    // answer, and Sonnet 5 defaults to spending part of max_tokens on
    // visible reasoning, which was eating the whole budget on tighter
    // limits and leaving zero tokens for the actual text/JSON output.

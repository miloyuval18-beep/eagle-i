...
// System-type-specific framing for an aging-system letter — plain-English
// name plus the kind of trouble that type of system tends to show once
// it's past its typical service life. Keys match SYSTEM_AGE_THRESHOLDS in
// routes/permits.js.
const SYSTEM_COPY = {
  roof: { label: 'roof', symptoms: 'leaks, missing or curling shingles, or granule loss' },
  hvac: { label: 'HVAC system', symptoms: 'rising energy bills, uneven cooling, or more frequent repairs' },
  water_heater: { label: 'water heater', symptoms: 'inconsistent hot water, rust-colored water, or a rumbling tank' }
};

const OPENERS_AGING = [
  ({ systemLabel, address, yearBuilt }) => `I came across public county records showing your home at ${address} was built around ${yearBuilt}, and wanted to reach out — homes from around that era are often due for a look at the ${systemLabel}.`,
  ({ systemLabel, address, yearBuilt }) => `Public records show your property at ${address} dates to around ${yearBuilt}. I'm reaching out because a home's original ${systemLabel} typically doesn't last forever, and it may be worth a quick check.`,
  ({ systemLabel, address, neighborhood }) => `As a business that works throughout ${neighborhood || 'the Houston area'}, I keep an eye on the age of homes in the area — which is how I came across your property at ${address}.`
];

const RELEVANCE_AND_CTA_AGING = [
  ({ companyName, systemLabel, symptoms, services, contact }) => `I'm with ${companyName}, where we focus on ${asFragment(services) || systemLabel + ' service and replacement'} for homeowners throughout the area. If you've noticed ${symptoms}, or just want a free, no-obligation inspection, I'd welcome the chance to take a look — you can reach me directly at ${contact}.`,
  ({ companyName, systemLabel, contact }) => `At ${companyName}, ${systemLabel} work is a core part of what we do. There's no cost or obligation to a quick inspection, and it's often easier to plan ahead than to deal with an emergency later — I can be reached directly at ${contact}.`,
  ({ companyName, systemLabel, unique, contact }) => `${companyName} has built its reputation on quality ${systemLabel} work${unique ? ', and ' + asFragment(unique) : ''}. I'd welcome the opportunity to take a look whenever it's convenient for you — you can reach me directly at ${contact}.`
];

// property: {address, zip, yearBuilt} — from lib/hcadZipValues.js's
//           getParcelAgesForZips(), a real HCAD-recorded construction
//           year, not an estimate.
// area:     {zip, region}
// tenant:   same shape as buildPermitLetter
// system:   one of SYSTEM_COPY's keys ('roof' | 'hvac' | 'water_heater')
// owner:    optional {firstName, lastName} — same "only if confidently
//           resolved" contract as buildPermitLetter's owner param
//           (lib/hcadZipValues.js's findConfidentOwners keys on the same
//           zip + normalized address, so the same lookup works here).
//
// Deliberately its own function, not a reuse of buildPermitLetter's
// OPENERS — those are written as "your permit for X, filed [date]..."
// which would be a false claim here: no permit exists, this is an
// inference from the property's age, and the letter has to say that
// honestly instead.
function buildAgingSystemLetter({ property, area, tenant, system, owner }) {
  const address = (property && property.address) || 'your property';
  const yearBuilt = (property && property.yearBuilt) || '';
  const neighborhood = (area && area.region) || '';
  const sys = SYSTEM_COPY[system] || SYSTEM_COPY.roof;
  ...
  const seed = hashString(`aging|${system}|${address}|${yearBuilt}`);
  const opener = pickVariant(OPENERS_AGING, seed)({ systemLabel: sys.label, address, neighborhood, yearBuilt });
  const relevanceAndCta = pickVariant(RELEVANCE_AND_CTA_AGING, seed + 7)({ companyName, systemLabel: sys.label, symptoms: sys.symptoms, services, unique, contact });

  return {
    recipientName,
    recipientAddress: address,
    zip: (area && area.zip) || (property && property.zip) || '',
    workLabel: `the ${sys.label}`,
    greeting,
    bodyParagraphs: [opener, relevanceAndCta],
    signOff: 'Sincerely,',
    signatureLines: [founder, companyName, [phone, email].filter(Boolean).join(' · ')].filter(Boolean)
...
module.exports = { describeWorkType, humanizeComments, buildPermitLetter, buildAgingSystemLetter };

...
// Returns { allowed, used, cap, unlimited }. Increments the counter only
// when allowed. A negative cap (-1) means unlimited — set on a tenant's
// tenants.<capColumn> to exempt them from that particular monthly limit
// entirely, still incrementing usage_counters for visibility but never
// gating on it. capColumn/counterColumn are internal constants (never user
...
async function checkAndIncrementCounter(tenantId, { capColumn, counterColumn }) {
  ...
  if (!unlimited && used >= cap) return { allowed: false, used, cap, unlimited };
  ...
  return { allowed: true, used: used + 1, cap, unlimited };
}
...
module.exports = { checkAndIncrementUsage, checkAndIncrementPlacesUsage, checkAndIncrementCounter, currentMonth };

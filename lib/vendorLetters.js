...
async function buildLetters({ tenantId, sourceKey, categoryKey, ids, message, includeWithEmail = false, includeRecentlyLettered = false, record = true }) {
  ...
  // Envelopes and labels are made from the same selection but aren't a second mailing.
  for (const l of (record ? letters : [])) {
    await query(

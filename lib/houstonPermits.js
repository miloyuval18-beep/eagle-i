...
// True when a permit's date falls in the same Mon-Sun week as `now` — used
// to flag a permit as "new this week" on the Permits page. Reuses the same
// ISO-week bucketing as detectPermitSpikes below, so "this week" means the
// same thing in both places. `now` is a parameter (not always `new Date()`
// internally) so this stays testable without mocking the clock.
function isCurrentWeek(dateStr, now = new Date()) {
  const key = isoWeekKey(dateStr);
  return key !== null && key === isoWeekKey(now);
}
...
module.exports = { getRecentPermits, detectPermitSpikes, isCurrentWeek };

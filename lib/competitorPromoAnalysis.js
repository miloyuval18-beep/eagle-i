...
function analyzeHtml(html, finalUrl) {
  ...
  return {
    ...
    isBuilderSubdomain: BUILDER_SUBDOMAIN_RE.test(host),
    host,
    wordCount,
    copyrightYear,
    title,
...
async function derivePromotionEdge(website) {
  ...
  if (s.isBuilderSubdomain) return {
    weakness: `Their site runs on a free website-builder address (${s.host}) instead of their own domain name — it can read as less established.`,

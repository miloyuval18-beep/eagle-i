...
async function screenRecipients(tenantId, recipients) {
  ...
  const [supp, sent] = await Promise.all([
    query('SELECT LOWER(email) AS e FROM outreach_suppressions WHERE tenant_id = $1 AND LOWER(email) = ANY($2)', [tenantId, emails]),
  ...
  const suppressed = new Set(supp.rows.map(x => x.e));
  const already = new Set(sent.rows.map(x => x.e));
  const stillOk = [];
  for (const c of candidates) {
    if (suppressed.has(c.email)) skipped.push({ ...c, reason: 'opted_out' });
...
async function sendOutreach({ tenantId, ctx, baseUrl, toEmail, vendorName, message }) {
  ...
  if (optedOut.rows.length) return { ok: false, reason: 'opted_out' };
  ...
  try {
    sent = await sendEmail({
      to: email, subject: `Quick note from ${ctx.companyName}`, html, text, replyTo, fromName: ctx.companyName,
  ...
  if (sendError) return { ok: false, reason: 'send_failed', error: sendError.message };
  return { ok: true, id: sent && sent.id ? sent.id : null };
}

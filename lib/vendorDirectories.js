...
const SOURCES = {
  ...
  supply: {
    ...
    where: (push, cat) => [
      `naics_code = ANY(${push(cat.param)})`, 'active = true', 'COALESCE(outlet_name, taxpayer_name) IS NOT NULL',
      `id = (SELECT MIN(t2.id) FROM comptroller_trades t2 WHERE t2.taxpayer_number = comptroller_trades.taxpayer_number AND t2.active AND t2.naics_code = ANY(${push(cat.param)}))`
    ],
    name: "COALESCE(NULLIF(outlet_name, ''), taxpayer_name)", location: 'outlet_city', locationLabel: 'City', phone: 'phone',
    since: 'COALESCE(first_sales_date, permit_issue_date)', expires: null,
    extra: "(CASE WHEN outlet_name IS NOT NULL AND taxpayer_name IS NOT NULL AND UPPER(outlet_name) <> UPPER(taxpayer_name) THEN 'Legal name: ' || taxpayer_name END)",
...
function buildWhere(source, cat, filters, tenantId, { includeUserFilters = true, sourceKey = null } = {}) {
  const params = [];
...
async function rankByDistance({ source, sourceKey, cat, filters, tenantId }) {
  ...
  const { where, params } = buildWhere(source, cat, filters, tenantId, { sourceKey });
...
async function listDirectory({ source: sourceKey, category, filters = {}, sort, limit = 40, offset = 0, tenantId }) {
  ...
  if (sortKey === 'distance') {
    ranked = await rankByDistance({ source, sourceKey, cat, filters, tenantId });
    if (!ranked) { sortKey = 'ready'; distanceNote = 'no_jobs'; }
  }

  const { where: baseWhere, params, push } = buildWhere(source, cat, filters, tenantId, { sourceKey });
  let where = baseWhere;

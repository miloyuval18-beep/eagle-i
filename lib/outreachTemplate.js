...
We are looking to build relationships with a few exceptional {plural} in {metro}, and we would like to introduce ourselves.
...
I wanted to follow up on my earlier note in case it got buried. We would still like to get to know a few exceptional {plural} in {metro}, and I would still like to buy you lunch or set up a quick call.
...
PITCH: "{blurb} I am looking to get to know a few exceptional {plural} in {metro}, since our clients often need someone they can trust. Do you have two minutes?"
IF INTERESTED: "I would like to buy you lunch or set up a call to introduce ourselves. Could you send me {ask}? What is the best email?"
IF NOT NOW: "No problem at all. May I check back in a few months?"
...
const DEFAULT_LINKEDIN_NOTES = {
  ...
  referral: 'Hi {name}, this is {founder} with {company}. I am looking to get to know a few exceptional {plural} in {metro}, since our clients often need someone they can trust. I would love to connect.',
...
// How the company's chosen market reads in a sentence (for example: the Dallas–Fort Worth area).
function metroPhrase(market) {
  return `the ${market && market.label ? market.label : 'Houston'} area`;
}

// Everything the template can say about this tenant + this vendor category.
function buildVariables({ tenant, profile, settings, source, category, market }) {
  const company = tenant.company_name;
  const area = (profile.service_area || '').trim();
  const metro = metroPhrase(market);
  const blurb = (settings.blurb || '').trim() || `${company} serves ${area || metro}.`;
  return {
    company,
    metro,
...
function templatize(message, vars) {
  ...
  const swaps = ['blurb', 'metro', 'plural', 'singular', 'ask', 'execute', 'linkedin', 'website']
...
module.exports = {
  ...
  cleanSite, normalizeLinkedin, metroPhrase, buildVariables, renderTemplate, templateFor, templatize, friendlyGreeting, fillName
};

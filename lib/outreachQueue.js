...
// recipients: [{ email, name, message, subject, variant, follow1Message, follow2Message,
// categoryKey, city, source, sourceId }] — every text already has the greeting filled in
// for that person. `testId` ties the two halves of a two-version test together.
async function enqueue({ tenantId, recipients, follow1Days = null, follow2Days = null, baseUrl, days, testId = null, now = new Date() }) {
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    const res = await query(
      `INSERT INTO outreach_queue (id, tenant_id, to_email, vendor_name, message, due_at, base_url,
                                   ...
                                   subject, variant, test_id, category_key, city, source, source_id)
      ...
      [crypto.randomUUID(), tenantId, r.email, r.name, r.message, due[i], baseUrl || '',
       ...
       r.subject || null, r.variant || null, testId, r.categoryKey || null, r.city || null, r.source || null, r.sourceId || null]
    );
  ...
  for (const q of claimed) {
    try {
      ...
      const res = await send({
        tenantId: q.tenant_id, ctx, baseUrl: q.base_url || '', toEmail: q.to_email, vendorName: q.vendor_name, message: q.message,
        subject: q.subject || undefined,
        meta: { kind: 'initial', variant: q.variant, testId: q.test_id, categoryKey: q.category_key, city: q.city, source: q.source, sourceId: q.source_id }
      });
      if (res.ok) {

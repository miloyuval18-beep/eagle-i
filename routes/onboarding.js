...
- leads: improve the business's own website so people who find it actually convert (fixing something Website Checkup would flag — no mobile support, no clear call-to-action, no contact form, etc.), or for real estate/home services specifically, reaching out to nearby property owners with recent building permits
...
For each one, write "fixSteps" as 2-4 short, concrete steps in plain language a non-technical business owner could follow with no guessing — each one a specific, literal action to take right now, not vague advice. Example of the level of detail wanted: "Open the Permits tab and select the 25 highest-value homes" rather than "target high-value areas." If following through means going to a specific place inside this app, set "whereTab" to exactly one of: market, competitors, opps, website_checkup, growth_vendors, networking, pr_awards, permits, budget, ads, social_hq, keywords, credentials — or "" if nothing in the app applies.
For each one, also set "eagleCanDoIt" to true only if this app could realistically send, post, or create the actual thing on the business's behalf with one more click — a social media post, a review-request email, an outreach email to a specific contact, or a paid ad campaign. Set it to false for anything that's really just research or navigating to a screen (e.g. "look at your competitors," "check your keyword report") and for anything this app has no tool for at all (e.g. hiring staff, changing prices, internal training, other operational changes) — those are real, worthwhile recommendations, they just don't get a "let Eagle I do it for you" button.
Important: never assume this business already has a Google Business Profile (the free Google Maps/Search listing) or has connected one, has a Facebook Page, or has any other social account already set up — you have no way of knowing that. If a step involves posting to or connecting one of these, phrase it so it works either way, e.g. "If you don't already have a free Google Business Profile, create one at google.com/business first (a few minutes) — then connect it in Eagle I's Credentials tab, then post an update." Same idea for Facebook/Instagram.
Return ONLY valid JSON: {"strategies":[{"priority":"high","category":"content","title":"Title","problem":"Problem","fixSteps":["Step 1","Step 2","Step 3"],"whereTab":"permits","eagleCanDoIt":true,"impact":"Numbers-based impact","timeline":"When"}]}`,
